...
R.objects.dir <- paste(paste(main.dir, "R_Objects", sep = "/"), "/", sep="")
...
if (!grepl("ENSDARG|ENSRNOG|ENSMUSG|ENSPANG|AGA|AAE|FBg", raw.dataset[2, 1])){
  if (length(list.files(IDs.crossRef.dir, pattern = "ID")) !=1){
    stop("ERROR: IDs.crossReference_file doesn't exist or isn't unique. Should be a unique file in /workspace_sampled/DATA/species/  directory")
  }
  ...
  rows.to.remove.R.object <- paste(R.objects.dir, "rows.to.remove_", species, "_", tissue, ".rds", sep="")
  rows.to.remove <- c()
  ...
  saveRDS(rows.to.remove, rows.to.remove.R.object)
