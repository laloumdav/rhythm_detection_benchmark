...
meta2d <- function(infile, outdir="metaout", filestyle, timepoints, minper=20,
            ...
            ARSdefaultPer=24,outRawData=FALSE,releaseNote=TRUE,outSymbol="", raw.pVal=FALSE)
{
    ...
    if (analysisStrategy == "auto") {
        if ( (non_integerInterval) | (uneven_interval) ) {
            ...
            if (CIRM["JTK"])
            {
                JTK_OUTM <- runJTK(EXP_dataframe, timepoints, minper=MINPER, maxper=MAXPER, raw.pVal=raw.pVal, releaseNote);
                outfile_tag["JTK"] <- 1;
            }
            if (CIRM["LS"])
            {
    ...
    if (  INTEGRATION != "noIntegration" )
    {
        ...
        if (integration_num > 1) {
            ...
            {
                if (nrow(pvaM) >= 2) {
                    cirpvaM <- getCirOmicsPVA(pvaM, method=combinePvalue, raw.pVal=raw.pVal);
                    if (adjustPhase == "notAdjusted")
                    {
