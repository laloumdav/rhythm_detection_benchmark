...
####======================================================================================================================================
runJTK <- function(indata,JTKtime,minper=20,maxper=28, releaseNote=TRUE, raw.pVal=raw.pVal)
{
	...
	jtkx <- function(z, ampci=FALSE, conf=0.8) {      ###v3.1 'ampci=TRUE' for calculating amplitude confidence
	  ...
	  if (isTRUE(raw.pVal)){            #### ADD OPTION TO KEEP RAW pvalues if raw.pVal=TRUE ####
	    padj <- unlist(pvals)
	  }
