...
# based method _and_ known periodicity frequency, the p-values.
{
  ...
  ## BEGIN REGRESSION APPROACH
	...
	for (gene in 1:noOfGenes){
		temp = tryCatch(rlm(x[,gene] ~ sine + cosine, method='MM'),     #### HERE we adapt the code to avoid error of dimension with method=MM in some cases ###
	...
	for (kierros in 1 : noOfPermutations){
	    ...
	    for (gene in 1:noOfGenes){
	        sarja = x[permu,gene]
	        temp = tryCatch(rlm(sarja ~ sine + cosine, method='MM'),     #### HERE we adapt the code to avoid error of dimension with method=MM in some cases ###
	                 warning = function(indpdt.var) { rlm(sarja ~ sine + cosine, method='M', maxit=60) },
	                 error = function(indpdt.var) { rlm(sarja ~ sine + cosine, method='M', maxit=60) })
	...
	for (gene in 1:noOfGenes) { #calculate p-values for each test/gene
		...
		if(pval[gene] == 0) {
		  pval[gene] = yDens[density_size, gene]
		}
...
#################################################################
fit.DC <- function(x){
	temp = tryCatch(rlm(x ~ 1, method='MM'),     #### HERE we adapt the code to avoid error of dimension with method=MM in some cases ###
	         warning = function(indpdt.var) { rlm(x ~ 1, method='M') },
	         error = function(indpdt.var) { rlm(x ~ 1, method='M') })
...
fit.freqs <- function(x,sine,cosine){
	temp = tryCatch(rlm(x ~ sine + cosine, method='MM'),     #### HERE we adapt the code to avoid error of dimension with method=MM in some cases ###
	         warning = function(indpdt.var) { rlm(x ~ sine + cosine, method='M') },
	         error = function(indpdt.var) { rlm(x ~ sine + cosine, method='M') })
...
fit.pi <- function(x,cosine){
	...
	temp = tryCatch(rlm(x ~ cosine + cosine, method='MM'),     #### HERE we adapt the code to avoid error of dimension with method=MM in some cases ###
	         warning = function(indpdt.var) { rlm(x ~ cosine + cosine, method='M') },
	         error = function(indpdt.var) { rlm(x ~ cosine + cosine, method='M') })
