...
for (a in 1:length(pvalues)) {
  ...
  if (length(colnames(pvalue.per.gene.1))>3){
    pvalue.per.gene.1 <- pvalue.per.gene.1[,c("ID", per.gene.normalization.method, "algorithm")]
  }
  ...
  if (length(colnames(pvalue.per.gene.2))>3){
    pvalue.per.gene.2 <- pvalue.per.gene.2[,c("ID", per.gene.normalization.method, "algorithm")]
  }
  ...
  conserved.genes.with.uniq.ortholog <- subset(merged.file, orthotype=="1:1" | orthotype=="n:1")
  conserved.genes.with.multiple.orthologs <- subset(merged.file, orthotype=="1:m" | orthotype=="n:m")
  ...
  conserved.genes.with.multiple.orthologs.normalized <- aggregate(conserved.genes.with.multiple.orthologs[ , paste(species2.name, "pvalue", sep="_")], 
                                                                         ...
                                                                         FUN = orthologs.group.normalization.method, drop=FALSE)
